...
/// URL 或标题任一命中即抑制（供 Feed / 透明模式 / 查找等 Native 侧门禁；含 google.com 等风险域）。
+ (BOOL)shouldSuppressPageAutomationForURL:(nullable NSURL *)url title:(nullable NSString *)title;

/// PagePack 专用门禁：仅人机挑战页（/sorry、CF、标题启发式），**不含** google.com 整站屏蔽。
/// 否则 Google Maps 等合法站点无法运行种子 Pack（如地图叠加校准）。
+ (BOOL)shouldSuppressPagePackForURL:(nullable NSURL *)url title:(nullable NSString *)title;
...
/// PagePack 注入包装用：仅人机挑战启发式，不按 google.com 等风险域整站静默。
