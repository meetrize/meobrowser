...
+ (NSString *)userScriptSourceBody {
    ...
    "  }\n"
    "\n"
    "  function reportTurnstileAndStop() {\n"
    "    var c = pickVisible('#cf-turnstile, .cf-turnstile, iframe[src*=\"challenges.cloudflare\"]') || { rect: null };\n"
    "    post({ event: 'detected', findings: [{ vendor: 'turnstile', kind: 'behavioral', confidence: 0.9, rect: c.rect, frame: 'iframe', detail: 'cloudflare turnstile' }] });\n"
    "    teardown();\n"
    "  }\n"
    "\n"
    "  function hasTurnstileDOM() {\n"
    "    try {\n"
    "      return !!document.querySelector('iframe[src*=\"challenges.cloudflare\"], #cf-turnstile, .cf-turnstile');\n"
    "    } catch (e) { return false; }\n"
    "  }\n"
    "\n"
    "  function scan() {\n"
    "    if (stopped) return;\n"
    "    // Turnstile 选择器也在通用静默函数里：先单独上报一次再停观察，避免挑战过程中持续 MutationObserver。\n"
    "    if (hasTurnstileDOM()) { reportTurnstileAndStop(); return; }\n"
    "    if (meoShouldSuppressCaptchaDetect()) { teardown(); return; }\n"
    "\n"
    "    var findings = [];\n"
    "\n"
