...
+ (NSString *)userScriptSource {
    ...
    "    }\n"
    "\n"
    "    var ocr = pickVisible('[data-meo-captcha=\"ocr\"], #meo-captcha-ocr, img.captcha-image, img[alt*=\"验证码\"], img[alt*=\"captcha\" i]');\n"
    "    if (ocr) {\n"
    "      findings.push({ vendor: 'generic', kind: 'text_ocr', confidence: 0.7, rect: ocr.rect, frame: 'main', detail: 'ocr image' });\n"
    "    }\n"
    "\n"
    "    var math = pickVisible('[data-meo-captcha=\"math\"], #meo-captcha-math');\n"
    "    if (math) {\n"
    "      findings.push({ vendor: 'generic', kind: 'math', confidence: 0.75, rect: math.rect, frame: 'main', detail: 'math challenge' });\n"
    "    }\n"
    "\n"
