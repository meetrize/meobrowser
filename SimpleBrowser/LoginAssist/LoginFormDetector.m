...
+ (void)installOnConfiguration:(WKWebViewConfiguration *)configuration
    ...
    // 主开关不在此固化：配置创建时的偏好会在刷新时被 UserScript 反复注入，覆盖用户关闭。
    // 一律先关，由 LoginAssistController 在导航完成 / 偏好变更时按当前偏好推送。
    ...
    NSString *source = [NSString stringWithFormat:
                        @"window.__meoLoginInlineEnabled=false;\n"
...
"  function inlineEnabled() { return window.__meoLoginInlineEnabled === true; }\n"
...
"      // 延后到下一个任务，避免 WK 消费 pointer 后 AppKit sheet/菜单点不动\n"
"      setTimeout(function() {\n"
"        post({\n"
...
"    // 勿在 pointerdown 上 preventDefault：否则 AppKit 收不到完整鼠标序列，\n"
"    // 随后弹出的 NSAlert sheet 按钮会无法点击。\n"
"    btn.addEventListener('click', function(e) {\n"
"      e.preventDefault();\n"
"      e.stopPropagation();\n"
"      setTimeout(function() { activateFieldButton(btn); }, 0);\n"
"    }, true);\n"
"  }\n"
...
"    let ctx = lastContexts.find(function(c) { return c.formId === formId; });\n"
"    if (!ctx && formId) {\n"
"      ctx = lastContexts.find(function(c) { return String(c.formId).indexOf(String(formId)) >= 0 || String(formId).indexOf(String(c.formId)) >= 0; });\n"
"    }\n"
"    if (!ctx) ctx = lastContexts[0];\n"
"    let fields = ctx ? collectFilledFields(ctx) : [];\n"
"    if ((!fields || !fields.length) && window === window.top) {\n"
"      for (let i = 0; i < window.frames.length; i++) {\n"
"        try {\n"
"          const f = window.frames[i];\n"
"          if (f && typeof f.__meoLoginAssistCollectFormFields === 'function') {\n"
"            const got = f.__meoLoginAssistCollectFormFields(formId);\n"
...
"    if (!inlineEnabled() || !focusModeEnabled() || !isPerField()) {\n"
"      focusSlot = null;\n"
"      return;\n"
"    }\n"
...
"  }\n"
"  window.__meoLoginAssistSetInlineEnabled = function(enabled, fromParent) {\n"
...
"        try {\n"
"          const f = window.frames[i];\n"
"          if (f && typeof f.__meoLoginAssistSetInlineEnabled === 'function') {\n"
"            f.__meoLoginAssistSetInlineEnabled(enabled, true);\n"
"          }\n"
"        } catch (e) {}\n"
"      }\n"
"    }\n"
...
"    if (!inlineEnabled() || !focusModeEnabled()) return;\n"
...
"  }, true);\n"
"  try {\n"
"    if (window !== window.top && window.top.__meoLoginInlineEnabled === true) {\n"
"      window.__meoLoginInlineEnabled = true;\n"
"    }\n"
"  } catch (e) {}\n"
"  schedule();\n"
"})();";
