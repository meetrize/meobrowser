...
+ (NSString *)userScriptSource {
    ...
    "function mkError(code,message){return{code:code,message:message||'',PERMISSION_DENIED:1,POSITION_UNAVAILABLE:2,TIMEOUT:3};}"
    "function mkPosition(p){return{coords:{latitude:p.latitude,longitude:p.longitude,accuracy:p.accuracy||0,"
    "altitude:p.altitude||null,altitudeAccuracy:p.altitudeAccuracy||null,heading:p.heading||null,speed:p.speed||null},"
    ...
    "if(payload&&payload.position){var pos=mkPosition(payload.position);if(cb.success){cb.success(pos);}if(cb.watch){cb.watch(pos);}return;}"
    ...
    "watchMap[watchId]=true;return watchId;"
    "};"
    "navigator.geolocation.clearWatch=function(watchId){delete watchMap[watchId];"
    ...
    "var id='perm-'+String(Date.now())+'-'+Math.random();"
    "pending[id]={success:resolve};"
    "post({type:'permissionQuery',requestId:id,host:host()});"
    "});"
    "}"
