...
| 分享 | 系统分享当前 URL |
| 发送到 Mac | 已连接时发 `open_url`；未连接提示；Mac 可反向「发送到手机」 |
...
确认：`{ "v": 1, "type": "open_url_ok" }`（可选带回 `url`）。

| 方向 | 触发 | 行为 |
|------|------|------|
| Android → Mac | ⋮「发送到 Mac」 | Mac `CompanionChannel` 鉴权后在前台窗口新开标签 |
| Mac → Android | 菜单「标签页」→「发送到手机」，或地址栏右侧「发送到手机」图标 | 手机新开标签；浏览器未在前台时拉起 `BrowserActivity` |
...
4. 标签数字正确；列表切换/关闭/左滑  
5. 新标签进入快捷方式页  
6. ⋮「发送到 Mac」与 Mac「发送到手机」在已连接时双向生效  
