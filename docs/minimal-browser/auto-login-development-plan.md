...
| Companion（V2） | **优先 Android App 短信自动推码**；Mac `waitOTP`+通道与 App 同波交付；粘贴/剪贴板为降级 |
| 中继 | 首版优先同 Wi‑Fi **Bonjour**；另提供自建 WS/Worker 模板；不做强制公有托管 |
...
建议节奏：V1 已完成；V2 **以 LA-5（含 Android Companion）为主门禁**，Mac 粘贴为降级；LA-4 TOTP 可在短信闭环后插入。详见 `.cursor/plans/login-assist-v2.plan.md`。
...
| Companion 工作量大 | App 严格最小（配对+推码）；Bonjour 优先；粘贴仅降级 | V2 以 Android 闭环为门禁 |
...
| 0.3 | 2026-07-15 | V2 定稿调整：Android Companion 短信推码优先；粘贴降级；Bonjour P0 |
