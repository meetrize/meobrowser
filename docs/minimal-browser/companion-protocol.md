...
> MeoBrowser（Mac）↔ MeoCompanion / Android MeoBrowser：局域网 OTP、通知镜像与可选数据同步。  
> 传输：Bonjour `_meologin._tcp` + 长度前缀 JSON。  
> 同步设计：[companion-sync-design.md](companion-sync-design.md) · 通知镜像：[companion-notification-mirror-design.md](companion-notification-mirror-design.md)
